...
room.onPlayerChat = (player, message) => {
    if (message == "!admin" && !admins.includes(player.id)) {
		...
		let lista = listaArtilheiros.toString()
		room.sendAnnouncement(lista)
	}
...
room.onTeamGoal = () => {  
  ...
  var fraseGol = frasesDeGol[indiceAleatorio];
  room.sendAnnouncement(`${fraseGol} é gol dele: ${string}`, undefined, 0xFF0000, 'bold')
}

room.onPlayerBallKick = (player) => {
	ultimoAChutar = player.name
	string = ultimoAChutar.toString() //converta o que foi recebido na linha 452 para string
	return [string, listadeGols.push({nome: player.name})]
}
